$(document).on('click', '.channel', function () {
   ...
   body.innerHTML = "";
   console.log('Click JQuery - this.id = '+this.id);
...
function reloadMsg(conv_id) {
   
       conversation_obj = $('#'+conv_id);
       //conversation_id.classList.remove("active");
       //document.getElementById("selectedChannel").value = ;
         
       
       var body = document.getElementById("chat-body");
       body.innerHTML = "";
       console.log('Pusher - conv_id = ' + conv_id);
       axios.post('/api/messages/' + user_id + '/' + conv_id).then(resp => {
           //console.log(resp.data);
       ...
       axios.post('/api/read/' + user_id + '/' + conv_id).then(resp => {
           //console.log('readed');
       });
       window.location.href = "#last";
       setTimeout(anchor, 500);
